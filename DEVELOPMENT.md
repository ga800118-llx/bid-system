...
② 后端保存文件（UUID 命名，不保留原文件名）
         ↓
③ 本地 PDFBox 提取全文（截断 8000 字符后发送）
         ↓
④ 截断文本 + Prompt 发送给 MiniMax /v1/chat/completions
   - 模型：MiniMax-M2.7
   - 返回：15 个字段的 JSON
         ↓
⑤ 解析 JSON，结果写入数据库
...
### API 信息
- 地址：https://api.minimaxi.com/v1/chat/completions
- 模型：MiniMax-M2.7
...
- 重要改动（截断量、Prompt、字段列表）必须同步更新本文档
...
| 日期 | 变更内容 |
|------|----------|
| 2026-03-30 | 确认 MiniMax 上传流程、文件大小 100M、计费一次=一文件、返回 JSON 格式、15 个提取字段、8000 字符截断 |

<template>
  <div class="admin-container">
    <div class="header">
      <el-button icon="Back" @click="goHome">杩斿洖棣栭〉</el-button>
      <h2>涓婁紶鎷涙爣鏂囦欢</h2>
      <div></div>
    </div>
    <div class="main">
      <el-card>
        <el-upload ref="uploadRef" drag :auto-upload="false" :limit="1" :on-change="handleFileChange" :file-list="fileList" accept=".pdf,.doc,.docx">
          ...
          <div>灏嗘枃浠舵嫋鍒版澶勶紝<span style="color:#409eff">鐐瑰嚮閫夋嫨</span></div>
          <template #tip><div class="el-upload__tip">鏀寔 PDF銆乄ord 鏂囨。锛屽崟涓枃浠朵笉瓒呰繃100MB</div></template>
        </el-upload>
        <div class="actions">
          <el-button type="primary" size="large" :loading="uploading" :disabled="!file" @click="handleUpload">涓婁紶骞舵彁鍙?/el-button>
          <el-button size="large" @click="goHome">鍙栨秷</el-button>
        </div>
        <div v-if="uploading" class="progress-area">
          <el-progress
            :percentage="progressPercent"
            :indeterminate="isExtracting"
            :format="(p) => isExtracting ? '姝ｅ湪鎻愬彇淇℃伅锛岃绋嶅€?..' : `涓婁紶涓?.. ${p}%`"
          />
        </div>
      </el-card>
      <el-card style="margin-top:24px">
        <template #header>
          <div style="display:flex;justify-content:space-between;align-items:center">
            <span>鏈€杩戜笂浼?/span>
            <el-button link @click="goHome" type="primary">鏌ョ湅鍏ㄩ儴</el-button>
          </div>
        </template>
        <el-table :data="recentProjects" stripe @selection-change="handleSelectionChange">
          ...
          <el-table-column label="寮€鏍囨椂闂? width="160">
            <template #default="{ row }">{{ row.bidOpenTime ? row.bidOpenTime.replace('T',' ').substring(0,16) : "-" }}</template>
          </el-table-column>
          <el-table-column prop="createdAt" label="褰曞叆鏃堕棿" width="160">
            <template #default="{ row }">{{ row.createdAt ? row.createdAt.replace('T',' ').substring(0,16) : "-" }}</template>
          </el-table-column>
          <el-table-column label="鎿嶄綔" width="120">
            <template #default="{ row }"><el-button link type="primary" @click="goDetail(row.id)">鏌ョ湅</el-button></template>
          </el-table-column>
        </el-table>
      </el-card>

    <div v-if="selectedIds.length > 0" class="batch-action-bar">
      <span class="batch-info">宸查€夋嫨 {{ selectedIds.length }} 椤?/span>
      <el-button @click="selectedIds = []; selectedRows = []">鍙栨秷</el-button>
      <el-button type="danger" @click="confirmBatchDelete">鎵归噺鍒犻櫎</el-button>
    </div>

    <el-dialog v-model="showDeleteDialog" title="纭鍒犻櫎" width="500px">
      <div style="margin-bottom: 12px;">纭畾瑕佸垹闄や互涓?{{ selectedIds.length }} 椤瑰悧锛熸鎿嶄綔涓嶅彲鎭㈠銆?/div>
      <div class="delete-list">
        ...
        <div v-if="selectedRows.length > 10" style="color: #999; font-size: 12px; margin-top: 6px;">
          杩樻湁 {{ selectedRows.length - 10 }} 椤?..
        </div>
      </div>
      <template #footer>
        <el-button @click="showDeleteDialog = false">鍙栨秷</el-button>
        <el-button type="danger" :loading="deleting" @click="handleBatchDelete">纭鍒犻櫎</el-button>
      </template>
    </el-dialog>
    </div>
  </div>
...
const handleUpload = async () => {
  if (!file.value) { ElMessage.warning("璇峰厛閫夋嫨鏂囦欢"); return }
  ...
  try {
    ...
    if (res.code === 200) {
      ElMessage.success("涓婁紶骞舵彁鍙栨垚鍔燂紒")
      router.push("/project/" + res.data.id)
    } else {
      ElMessage.error(res.msg || "涓婁紶澶辫触")
    }
  } catch (e) {
    ElMessage.error("涓婁紶澶辫触: " + (e.response?.data?.msg || e.message))
  } finally {
...
const goHome = () => router.push("/projects")
